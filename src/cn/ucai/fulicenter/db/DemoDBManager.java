...
public class DemoDBManager {
    ...
    synchronized public void saveAvatar(UserAvatar avatar){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(UserDao.USER_COLUMN_NAME_ID, avatar.getMUserName());
        values.put(UserDao.USER_COLUMN_ID_AVATAR, avatar.getMAvatarId());
        values.put(UserDao.USER_COLUMN_NAME_NICK, avatar.getMUserNick());
        values.put(UserDao.USER_COLUMN_PATH_AVATAR, avatar.getMAvatarPath());
        values.put(UserDao.USER_COLUMN_TYPE_AVATAR, avatar.getMAvatarType());
        values.put(UserDao.USER_COLUMN_LAST_UPDATE_TIME, avatar.getMAvatarLastUpdateTime());
        if(db.isOpen()){
    ...
    public UserAvatar getUserAvatar(String username) {
        ...
        if (cursor.moveToNext()) {
            ...
            user.setMAvatarId(cursor.getInt(cursor.getColumnIndex(UserDao.USER_COLUMN_ID_AVATAR)));
            user.setMAvatarType(cursor.getInt(cursor.getColumnIndex(UserDao.USER_COLUMN_TYPE_AVATAR)));
            user.setMAvatarPath(cursor.getString(cursor.getColumnIndex(UserDao.USER_COLUMN_PATH_AVATAR)));
            user.setMAvatarLastUpdateTime(cursor.getString(cursor.getColumnIndex(UserDao.USER_COLUMN_LAST_UPDATE_TIME)));
            return user;
        }
        return user;
